...
const PlayerStatus = () => {
  ...
  return (
    <Card className="w-full h-full col-span-2 border border-gray-800 bg-gray-800/50 rounded-md p-0 overflow-hidden mb-6">
      <div className="w-full flex flex-col gap-1">
        ...
        <div className="h-full max-h-96 overflow-y-auto scrollbar-hidden">
          {status.map((player, index) => (
            <div
              key={player.id}
              className="w-full flex flex-col gap-4 p-4 py-4 hover:bg-gray-900/50 transition-all duration-300 border-b border-gray-500/20 last:border-none"
            >
              {/* Player Info */}
              <div className="w-full flex items-center justify-between">
                <p className="text-gray-300 font-medium">{player.name}</p>
