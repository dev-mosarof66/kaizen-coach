...
const Pagination = ({ totalItems = 127, itemsPerPage = 8 }) => {
    ...
    return (
        <div className='w-full flex flex-col gap-2 sm:flex-row items-center justify-between py-4 px-4'>
            <p className='text-sm sm:text-xs xl:text-xs text-gray-300'>
                {t('tasksPage.pagination.showing')} {(currentPageIndex - 1) * itemsPerPage + 1} -{' '}
                {Math.min(currentPageIndex * itemsPerPage, totalItems)} {t('tasksPage.pagination.of')} <span>{totalItems}</span> {t('tasksPage.pagination.tasks')}
            </p>
